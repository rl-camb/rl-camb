...
class VPGSolver(StandardAgent):
    ...
    def solve(self, env_wrapper, max_iters, verbose=False, render=False):
        ...
        for batch_num in range(max_iters):
            ...
            state_act_batch, batch_future_rewards = [], []  # Whole batch

            for step in itertools.count():
                if render:
                    env.render()
                action = self.act(state, epsilon=None)
                ...
                state_act_batch.append( (state.copy(), np.int32(action)) )
                ...
                if done:
                    ...
                    if len(state_act_batch) > self.batch_size:
                        break
                    else:
            ...
            self.learn(state_act_batch, batch_advs)
            ...
            if batch_num % 25 == 0 or solved:
                print(f"\rEpisode {batch_num + 1}/{max_iters} "
    ...
    def learn(self, state_act_batch, advantage_batch):
        ...
        """
        sts, acts = map(tf.convert_to_tensor, zip(*state_act_batch))
        advs = tf.convert_to_tensor(advantage_batch)
...
class VPGSolverWithMemory(VPGSolver):
    ...
    def __init__(self, memory_len):
        # TODO create a deque memory
        pass

    def save_state(self):
        # TODO add memory to the save state
        pass
